...
> - **授权状态**：截至 2026-08-25，仓库现有核查未发现上述接口的公开开发者文档、API Key 申请、SLA、版本承诺或明确的调用、缓存及再展示授权，本项目也尚未取得书面许可。因此，可访问不等于已获授权，当前状态不足以确认依赖这些接口的版本适合公开运营或商业化。现有实现仅由用户操作触发精确期号的低频查询，不进行后台抓取或批量归档，官网原始响应不落盘；这些限制用于降低风险，但不替代授权。
...
| V1.3 | **进行中** | 跨平台走势图、历史开奖存储和数学预测 | 走势图优先开发；持久化待数据授权，预测待冻结回测方案 |
| V1.4 | **未完成** | 基于 V1.3 历史数据的大模型分析 | 依赖 V1.3，待完成 AI 供应商与合规设计 |
| V1.5 | **未完成** | 传统文化娱乐推演 | 概念规划，待隐私与合规专项评审 |

### 当前状态

V1.3 已进入开发。当前先实现 Android/iOS 共用的官方常见基本走势矩阵和遗漏统计，并用受控快照完成界面与领域验收；未取得允许长期存储、分析和再展示历史开奖数据的授权前，不批量抓取、不新增历史开奖数据库，也不会把演示数据标记为实时官方结果。数学预测将在走势图稳定后单独冻结算法与样本外回测口径。
