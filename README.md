...
| B4 移动票面解析与人工校正 | 进行中 | 已实现彩种/期号校正、逐注号码球、倍数步进、追加选择、金额推导、实时领域错误及 `ConfirmedTicket` 来源追踪；期号、倍数、追加或金额缺失及属性矛盾可携带安全草稿进入校正页；已支持原图预览及期号、逐注号码、倍数、追加、金额区域切换和高亮定位 | 低置信字段标记、期号/金额多候选处理和足量真实票样验收未完成 |
| B5 移动核心端到端闭环 | 核心完成，待验收 | Android/iOS 已接入精确期号官网查询、本地规则计算、逐注结果、奖金终态、双源证据展示及保票据重试 | B2 正式对账与授权、足量真实票样、移动真机完整结果交互和发布窗口验收未完成 |
...
当前自动化基线共 451 项：JVM 155 项、Android Host 152 项、iOS Simulator 144 项，失败、错误和跳过均为 0。Android 和 iOS 核心相机流程已通过真机冒烟；满足当前保守解析条件的真实图片，以及结构安全但缺失期号、倍数、追加或金额及属性矛盾的 OCR 结果可以进入人工校正，并对照原图定位已有字段。倍数和追加未知时不会填入默认值，必须由用户明确选择；仅在图片预览可用且领域校验通过后生成 `ConfirmedTicket`。
