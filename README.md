...
| B4 移动票面解析与人工校正 | 进行中 | 已实现彩种/期号校正、逐注号码球、倍数步进、追加开关、金额推导、实时领域错误及 `ConfirmedTicket` 来源追踪；期号或金额缺失、金额矛盾可携带安全草稿进入校正页；已支持原图预览及期号、逐注号码、金额区域切换和高亮定位 | 倍数/追加独立定位、低置信字段标记、多候选选择和足量真实票样验收未完成 |
...
当前自动化基线共 427 项：JVM 147 项、Android Host 144 项、iOS Simulator 136 项，失败、错误和跳过均为 0。Android 和 iOS 核心相机流程已通过真机冒烟；满足当前保守解析条件的真实图片，以及结构安全但缺失期号、缺失金额或金额矛盾的 OCR 结果可以进入人工校正，并对照原图定位期号、逐注号码和金额，仅在图片预览可用且领域校验通过后生成 `ConfirmedTicket`。开奖查询仍使用明确标注的固定演示数据，尚不能用于真实中奖判断；确认或退出后会清理应用私有临时图片。
