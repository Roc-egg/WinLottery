...
<div align="center">
  ...
  <p>
    ...
    <img alt="项目状态" src="https://img.shields.io/badge/status-V1.3%20completed-2f7d32">
...
更新日期：2026-08-26。当前稳定版本为 `1.3.0 (5)`，V1、V1.1、V1.2 与 V1.3 均已按各自个人自用基线完成关闭；V1.4 与 V1.5 尚未开始。
...
V1.3 已完成版本关闭。Android/iOS 共用的基本走势以官网最新已发布期为锚点提供 `50 / 80 / 120 / 300 / 500` 期真实样本；数学研究使用固定策略、50 期训练窗口和 450 期时间前推回测，并如实对比精确均匀随机基线。ADR-024 已明确：公开开奖事实的用户触发、低频、当前会话使用不阻止开源个人自用版本关闭；官网接口没有开放 API 与 SLA 只作为稳定性和未来运营边界。长期历史数据库不属于当前需求，V1.4 若需要历史快照必须重新冻结数据与存储方案。
