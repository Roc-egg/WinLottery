...
| V1.3 | **进行中** | 跨平台走势图、历史开奖存储和数学预测 | 会话内真实走势图已接入；持久化待数据授权，预测待冻结回测方案 |
| V1.4 | **未完成** | 基于 V1.3 历史数据的大模型分析 | 依赖 V1.3，待完成 AI 供应商与合规设计 |
| V1.5 | **未完成** | 传统文化娱乐推演 | 概念规划，待隐私与合规专项评审 |

### 当前状态

V1.3 已进入开发。Android/iOS 共用的官方常见基本走势矩阵和遗漏统计已经接入真实官网历史开奖，以最新已发布期为锚点提供 `50 / 80 / 120 / 300 / 500` 期；每个彩种首次进入时加载一次并只保留在当前会话内。未取得允许长期存储、分析和再展示历史开奖数据的授权前，不新增历史开奖数据库、不后台轮询、不长期归档。数学预测将在走势图稳定后单独冻结算法与样本外回测口径。
