...
<div align="center">
  ...
  <p>
    <img alt="当前版本" src="https://img.shields.io/badge/version-1.1.0-2f7d32">
    <img alt="项目状态" src="https://img.shields.io/badge/status-V1.2%20in%20progress-1565c0">
...
更新日期：2026-08-25。当前稳定版本为 `1.1.0 (3)`，V1 与 V1.1 已完成关闭，V1.2 已立项并进入开发。
...
| V1.2 | **进行中** | 大乐透、双色球合法随机号码，支持期数和每期注数 | 领域边界已冻结，正在实现 UI 与双端验收 |
...
V1.2 正在推进合法随机选号和面向后续版本的一级导航重构。已冻结 `1..20` 期、每期 `1..10` 注、单次最多 `200` 注的边界；大乐透跨期只复用同一批号码，双色球可选择复用或逐期独立随机。
...
V1.1 与 KMP 结构迁移后的最新基线已经完成：

- Gradle 格式检查、JVM、Android Host、iOS Simulator 测试和 Android Debug/Release/AAB 构建，321 个任务全部通过。
- 固定 Android Emulator 与 iOS Simulator 回归，151 个任务全部通过。
- iOS Simulator Release Xcode 完整构建通过。
- `1.0.1 (2)` 至 `1.1.0 (3)` 的真实 Room 覆盖升级验证通过，结构化记录完整保留。
...
| [V1.2 推进记录](docs/delivery/V1.2-progress.md) | 随机选号领域契约、UI 重构和当前验收状态 |
