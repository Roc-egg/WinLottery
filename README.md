...
| B2 开奖查询与规则引擎 | 核心完成，待验收 | 已实现体彩/福彩适配器、双源证据状态、两种彩票现行规则、倍数和大乐透追加计算；移动端手动单式录入已绕过 OCR 接入同一查询闭环 | 每种彩票 20 期正式对账、连续 6 个开奖窗口采样和数据授权仍未完成 |
...
| B5 移动核心端到端闭环 | 核心完成，待验收 | Android/iOS 已接入拍照/OCR 与手动录入两种入口、精确期号官网查询、本地规则计算、逐注结果、奖金终态、双源证据展示及保票据重试；两端模拟器已完成实体票真实未中奖、构造一等奖、未来期号安全阻断、最大辅助字号和断网恢复验收 | B2 正式对账与授权、足量真实票样、移动物理真机完整结果交互和真实发布窗口验收未完成 |
...
当前自动化基线共 466 项：JVM 159 项、Android Host 156 项、iOS Simulator 151 项，失败、错误和跳过均为 0。Android 和 iOS 核心相机流程已通过真机冒烟；满足当前保守解析条件的真实图片，以及结构安全但缺失期号、倍数、追加或金额及属性矛盾的 OCR 结果可以进入人工校正，并对照原图定位已有字段。倍数和追加未知时不会填入默认值，必须由用户明确选择；仅在图片预览可用且领域校验通过后生成 `ConfirmedTicket`。
