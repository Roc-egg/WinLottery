...
| B3 图片采集与 OCR PoC | 进行中 | Android/iOS 拍照、系统选图、本地 OCR 和保守解析已贯通；桌面导图、无遥测 ONNX 工作进程、PP-OCRv5 来源锁定、可复现转换及数值一致性验证已完成 | 桌面模型分发与真实推理、ZXing、Windows 工作进程实机验收、完整图片质量与几何校正、准确率统计和干净系统连续识别未完成 |
...
当前自动化基线按现有 Gradle 模块统计为 356 项：JVM 124 项、Android Host 120 项、iOS Simulator 112 项，失败、错误和跳过均为 0。Android 和 iOS 核心相机流程已通过真机冒烟；真实图片不会进入 Fake 开奖流程，分析结束后会清理应用私有临时副本。

> 当前版本不能用于真实中奖判断，也不能解释为支持任何省份、销售终端或票面版式。B2、B3 均未通过验收闸门。

桌面端已完成 ONNX Runtime 底座、独立本地工作进程，以及 PP-OCRv5 三段官方 Paddle 模型的来源锁定、opset 17 转换和 Paddle/ONNX 合成张量逐元素对比。转换后的约 22.3 MB 模型二进制尚未纳入仓库或安装包，图片预处理、后处理和工作进程推理协议也未实现；普通 UI 主进程仍不加载 ONNX，真实桌面 OCR 继续保持明确阻断。
