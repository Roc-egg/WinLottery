...
当前自动化基线共 439 项：JVM 151 项、Android Host 148 项、iOS Simulator 140 项，失败、错误和跳过均为 0。Android 和 iOS 核心相机流程已通过真机冒烟；满足当前保守解析条件的真实图片，以及结构安全但缺失期号、倍数、追加或金额及属性矛盾的 OCR 结果可以进入人工校正，并对照原图定位已有字段。倍数和追加未知时不会填入默认值，必须由用户明确选择；仅在图片预览可用且领域校验通过后生成 `ConfirmedTicket`。开奖查询仍使用明确标注的固定演示数据，尚不能用于真实中奖判断；确认或退出后会清理应用私有临时图片。
