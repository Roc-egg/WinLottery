...
| B3 图片采集与 OCR PoC | 进行中 | Android/iOS 拍照、系统选图、本地 OCR 和保守解析已贯通；桌面导图、去元数据副本、分辨率闸门和临时文件清理已完成 | 桌面 PP-OCR/ZXing、完整图片质量与几何校正、置信度校准、准确率统计和干净系统连续识别未完成 |
...
当前自动化基线为 353 项：JVM 121 项、Android Host 120 项、iOS Simulator 112 项，失败、错误和跳过均为 0。Android 和 iOS 核心相机流程已通过真机冒烟；真实图片不会进入 Fake 开奖流程，分析结束后会清理应用私有临时副本。
