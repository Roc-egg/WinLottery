...
| B5 移动核心端到端闭环 | 核心完成，待验收 | Android/iOS 已接入拍照/OCR 与手动录入两种入口、精确期号官网查询、本地规则计算、逐注结果、奖金终态、双源证据展示及保票据重试；两端模拟器已完成实体票真实未中奖、按真实开奖号码构造的一等奖及未来期号安全阻断验收 | B2 正式对账与授权、足量真实票样、移动物理真机完整结果交互和真实发布窗口验收未完成 |
...
当前自动化基线共 463 项：JVM 159 项、Android Host 156 项、iOS Simulator 148 项，失败、错误和跳过均为 0。Android 和 iOS 核心相机流程已通过真机冒烟；满足当前保守解析条件的真实图片，以及结构安全但缺失期号、倍数、追加或金额及属性矛盾的 OCR 结果可以进入人工校正，并对照原图定位已有字段。倍数和追加未知时不会填入默认值，必须由用户明确选择；仅在图片预览可用且领域校验通过后生成 `ConfirmedTicket`。
...
2026-08-14 已在 Android 16 和 iOS 26.5 模拟器使用同一张实体双色球票完成真实结果交互验收：精确查询 `2026092` 期，5 注、1 倍、合计 10.00 元，两端均取得奖金终态和两份中国福彩网证据，逐注结果一致为未中奖；正常重查保持同一期同一结论。Android 另已验证断网重查只显示网络不可用，不形成中奖或未中奖结论，恢复联网后可重新取得相同真实结果。含实体票号码的验收截图只保存在本机临时目录，不纳入仓库。
