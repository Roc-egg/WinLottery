...
readonly APPIUM_CHECK_SCRIPT="$SCRIPT_DIRECTORY/v1-2-random-number-appium-check.mjs"
readonly APPIUM_PORT=4728
...
[[ -f "$APPIUM_CHECK_SCRIPT" ]] || current_vm_fail "缺少 V1.2 Appium 检查脚本"
...
readonly WORK_DIRECTORY="$(mktemp -d "${TMPDIR:-/tmp}/winlottery-v1-2.XXXXXX")"
...
run_appium_phase() {
  ...
  if ! node \
    ...
    print -u2 "V1.2 $phase 双虚拟机验收失败，Appium 日志末尾如下："
...
for _ in {1..100}; do
  ...
  if ! kill -0 "$APPIUM_PROCESS_ID" 2>/dev/null; then
    print -u2 "V1.2 验收 Appium 提前退出"
    tail -n 160 "$APPIUM_LOG" >&2 || true
    exit 1
  fi
  sleep 0.1
done
curl --silent --show-error --fail "$APPIUM_BASE_URL/status" >/dev/null || current_vm_fail "等待 V1.2 Appium 就绪超时"
...
print "双虚拟机 V1.2 标准与最大字号、200 注边界、逐期独立、重新生成和会话保留检查通过"
