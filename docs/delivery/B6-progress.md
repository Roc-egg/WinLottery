# B6 Android/iOS V1 个人自用收尾记录

记录日期：2026-08-24

## 状态结论

根据 ADR-018，V1 已按个人自用内部基线完成收尾，B6 正式关闭。用户已在 Android 实体设备完成 V1 运行验收；此前双虚拟机开发验收也已固化 Android/iOS Release 配置的可重复构建、非 Debug 属性、覆盖安装、启动、初始化清扫、Android 相机首次拒权、受控进程终止、`SIGKILL`、Debug 受控崩溃恢复、低内存开发回调边界、卸载清除、构建号 1 → 2 开发签名升级检查、固定 12MP 重复采样开发基线、横竖屏开发检查、受控网络抖动开发检查，以及最大辅助字号下的 V1 支持范围、手动录入可达性与退出状态清空检查。

当前结论是个人自用 V1 的关闭结论，不是公开发布验收。用户单台 Android 真机结果与固定双虚拟机回归共同证明当前目标范围可用，但不构成完整机型矩阵或性能承诺。正式生产签名、商店上传和公开发布材料不在当前产品目标内；未来若改变分发方式，再单独重启对应计划。

后续维护回归目标仍严格锁定为 Android `Medium_Phone_API_36.1`（`emulator-5554`）和 iPhone 17 Pro / iOS 26.5 Simulator（`E6D04C31-9EB8-4696-990F-DA54D0112E13`）。共享守卫会核对 Android 模拟器标志与 AVD 名称，以及 iOS Simulator 的 UDID、名称、启动状态和运行时；任一目标不匹配时立即失败，不回退到其他 ADB 或 Apple 设备。
...
- 关于页 V1 支持范围已与现有能力对齐：明确支持 1 至 20 期受控连续投注，补打票在号码、期号、期数、倍数、追加属性和金额完整闭环时支持；超过 20 期与跨年度未知期次仍列入未支持范围。共享契约测试防止页面重新退回“全部多期不支持”或“全部补打票不支持”的旧描述。
- 新增 `tools/mobile/run-current-vm-v1-scope-checks.sh` 和对应 Appium 检查。专项先复用完整 Release 构建、产物、安装和启动闸门，再把锁定 Android 模拟器临时切换到 `2.0` 字号、锁定 iOS Simulator 临时切换到 `accessibility-extra-extra-extra-large`。脚本从首页进入关于页并按需真实滚动，核对支持、限制和验真边界文案；随后进入空白手动录入，真实选择彩种和基本投注、输入本地合成期号 `26999` 与金额 `4.00`、把期数从 1 增加到 2、完成键盘操作并触达底部确认入口，再返回首页重新进入并核对旧状态已经清空。Android 使用 IME `done`，iOS 使用应用内完成按钮；共享页面通过 IME 内边距避让键盘，键盘显示时的滚动只从上方内容区发起。所有目标控件都必须由 WebDriver 报告为可见且完整位于窗口内，Android 与 iOS 分别按 `checked` 和 `selected` 确认选择态；iOS 滚动后会等待元素矩形稳定，期数加号只发送一次点按并严格等待 `2 期`，不会用重复点按掩盖状态延迟。成功、失败和退出路径都会关闭会话、恢复原字号与应用前台状态，并删除 Appium 日志。
...
- 最大字号输入与键盘避让专项最终重新执行 Android Release 的 145 个 Gradle 任务并全部成功，iOS `Release-iphonesimulator` 完整构建、覆盖安装和非 Debug 启动检查通过。Android 关于页滚动 1 次、手动录入累计滚动 3 次；iOS 关于页无需滚动、手动录入累计滚动 2 次。两端均确认期号 `26999`、金额 `4.00` 和 `2 期` 已实际写入，Android IME `done` 与 iOS 应用内完成按钮均收起键盘，键盘遮挡控件不会通过可见性断言；返回首页再次进入后，输入值、期数和彩种状态全部清空。临时验收签名 APK 为 59,366,902 字节，SHA-256 为 `08dd409ebf62902e5b522b29606cce06b815523ce07557d544429e7030f1c979`；未签名 AAB 为 34,323,474 字节，SHA-256 为 `9409b51fdde77f373afe15bd3cd5cc6d062754efbfe8d18555e4d4c90c05dec3`；iOS Simulator `.app` 为 41,604 KiB。退出后双端字号和应用前台状态已恢复，临时 Appium 服务、日志与 Release 验收目录均已清理。
- 最大字号输入与键盘避让增量完成后复跑固定双虚拟机回归：iOS Simulator 231/231，Android 数据层 67/67、识别层 60/60，失败、错误和跳过均为 0；140 个 Gradle 任务全部实际执行成功。专项脚本通过 Node 与 Zsh 语法检查，完整差异通过 `git diff --check`。
- 最大字号双彩种手动录入专项再次执行 Android Release 的 145 个 Gradle 任务并全部成功，iOS `Release-iphonesimulator` 完整构建、覆盖安装和非 Debug 启动检查通过。两端在完成大乐透输入和第一次状态清空后，均真实选择双色球并遍历红球 `0/6`、蓝球 `0/1`、倍数、期数、金额和底部确认入口；任一页面快照出现大乐透“追加投注”都会失败。第二次返回首页并第三次进入后，双色球投注区同样清空。Android 与 iOS 手动录入均累计滚动 5 次；Android 使用绑定当前 AVD 会话的原生输入命令，不依赖键盘重排后可能失效的元素标识。临时验收签名 APK 为 59,366,902 字节，SHA-256 为 `0c4956c0669ad90803fd21d1384e6421e3d9bb5b0c0c12ba762d10cfd6ce4e37`；未签名 AAB 为 34,323,474 字节，SHA-256 为 `f6fe3dbe72e373f0adfa35c03911cbc8c0c95a619e01366fc8d4e38343a64c06`；iOS Simulator `.app` 为 41,604 KiB。退出后双端字号和应用前台状态已恢复，临时服务与目录均已清理。
- 双彩种最大字号验收增量完成后复跑固定双虚拟机回归：iOS Simulator 231/231，Android 数据层 67/67、识别层 60/60，失败、错误和跳过均为 0；140 个 Gradle 任务全部实际执行成功。

## 个人自用范围外与能力边界

- Android 生产签名、密钥托管、签名轮换方案、Play App Signing 和 Play Console 上传验证未执行，不属于当前个人自用目标。
- iOS Archive、分发证书、Provisioning Profile、正式 IPA 导出、安装和 App Store Connect 上传验证未执行，不属于当前个人自用目标。
...
- B2 第二人独立真值复核、连续 6 个开奖窗口采样和官方数据商用授权未完成，只限制对外可靠性及未来公开分发声明。
- 应用商店隐私清单、截图文案、年龄与彩票相关商店政策核对未执行，不属于当前个人自用目标。

## 下一步

1. 启动 V1.1 基线冻结：确定 Room 3 准确依赖、数据库模式、迁移策略、备份策略和跨平台逻辑导入导出格式。
2. V1 只保留必要回归，不再追加收尾批次；生产签名、完整真机矩阵、商店上传与公开发布材料不进入 V1.1 计划。
3. B0/B2/B3 的票样真值、开奖窗口和数据授权继续作为能力声明边界；缺少这些材料时保持白名单为空、人工确认和失败关闭，但不阻止 V1.1 实施。
