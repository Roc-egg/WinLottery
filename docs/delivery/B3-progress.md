...
B3 已打通 Android 和 iOS 的“系统选图或拍照 → 私有去元数据副本 → 本地 OCR → 共享保守解析 → 人工校正页”纵向链路；Android CameraX 和 iOS AVFoundation 的核心拍照流程均已通过真机闭环。iOS 横竖屏、闪光灯模式和拒绝权限分支仍待补测。移动 B3 尚未通过发布验收，图片质量与几何校正、真值标注和准确率统计仍未完成。
...
- 原图区域与低置信度字段对照、解析候选值和置信度聚合仍属于 B4 未完成范围；号码编辑器和人工确认已实现。
...
4. 在 Android/iOS 真机验证人工校正页、字段来源、领域错误、确认闸门和临时图片清理。
5. 配合已完成的 B5 真实开奖闭环，在 Android/iPhone 物理真机复验“拍照或导入 → OCR/原图辅助录入 → 确认 → 真实结果”的完整流程。
