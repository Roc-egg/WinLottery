...
- 当前可采集期号、逐注号码、倍数、追加和票面金额区域的原始分数；OCR 引擎名称与字段分数只存在于当前校正流程内存，不写日志、不持久化，也不进入确认或查询决策。
...
- 原图字段区域、解析候选值和原始置信度聚合已经接入 B4；Android/iOS 分别校准、低置信视觉标记和足量真值仍未完成。
...
- 该 B3 批次当时的自动化测试共 524 项：JVM 181 项、Android Host 174 项、iOS Simulator 169 项；失败、错误和跳过均为 0。新增覆盖四侧印刷内容贴边裁切、受损 OCR 摘要、漏分区符和纵排文字跨行干扰；当前总基线见项目 README。
- Android Debug APK：62,255,464 字节，SHA-256 `56e8ac30a30903a7400f3205c95f4fa61e9fce5a357ccaea7db7d2868ae59883`。
