...
B3 已打通 Android 和 iOS 的“系统选图或拍照 → 私有去元数据副本 → 分辨率与像素质量闸门 → 本地 OCR → 共享保守解析 → 人工校正页”纵向链路；Android CameraX 和 iOS AVFoundation 的核心拍照流程均已通过真机闭环。移动端现可保守拒绝严重整体过暗、严重整体过亮和明显全局模糊图片，两端模拟器已完成清晰图放行、三类异常互斥提示及临时图片清理验收。iOS 横竖屏、闪光灯模式和拒绝权限分支仍待补测。移动 B3 尚未通过发布验收，裁切、透视、阴影、局部反光、真值标注和准确率统计仍未完成。
...
- 曝光问题优先于模糊返回，单张图片只提供一个当前最可操作的提示。13 张探索图的正常清晰度最低约为 788，确定性强模糊变体最高约为 22；当前阈值刻意留出较大间隔，但仍需真实多条件拍摄真值验证。
...
- Android 16 和 iOS 26.5 模拟器分别使用同一张清晰探索图及其确定性模糊、过暗、过亮变体完成运行验收。清晰图继续进入各自本地 OCR；三类异常分别只出现对应提示，没有交叉误报，返回首页后两端私有临时票图目录均为空。
- iOS 过亮提示作为最长文案完成 402 像素宽界面截图检查，Android 错误页也完成视觉检查；标题、说明和返回操作均无裁切或重叠。验收截图与变体只保存在本机临时目录，不纳入仓库。
...
- 裁切、透视、阴影、局部反光及更复杂的局部或运动模糊检测；当前只覆盖严重整体曝光异常和明显全局模糊。
...
- 自动化测试共 503 项：JVM 176 项、Android Host 166 项、iOS Simulator 161 项；失败、错误和跳过均为 0。新增覆盖共享指标和检查链、Android 缺失文件安全失败，以及 iOS 有效 JPEG 灰度解码。
- Android Debug APK：62,129,674 字节，SHA-256 `089b97ab5b53384ea7816841bcd3ca20717198ced91a5b76c52aa4085bd62ec6`。
- Xcode `iosApp` Scheme 已针对 iPhone 17 Pro、iOS 26.5 Simulator 完整构建成功；当前 `.app` 安装后完成四类图片质量运行验收。
- 本轮未重新构建 Windows/macOS 分发包；桌面 OCR 底座继续按 ADR-011 保留并延期，不属于 Android/iOS V1 当前回归范围。
- 以上均为开发构建基线，包含调试符号，不能作为发布包体积承诺。

## 移动 V1 下一步

1. 建立清晰、运动模糊、失焦、欠曝、过曝和正常白底票面的脱敏质量真值集，统计 Android/iOS 误拒率与漏检率后冻结阈值。
2. 实现彩票边缘、裁切、旋转和透视检测，再评估阴影、局部反光与增强候选图，所有失败继续提供单一可操作提示。
