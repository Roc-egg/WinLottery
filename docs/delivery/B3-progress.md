...
B3 已打通 Android 和 iOS 的“系统选图或拍照 → 私有去元数据副本 → 分辨率与像素质量闸门 → 本地 OCR → 共享保守解析 → 人工校正页”纵向链路；Android CameraX 和 iOS AVFoundation 的核心拍照流程均已通过真机闭环。移动端现可保守拒绝严重整体过暗、严重整体过亮、明显全局模糊和明显画面倾斜，两端模拟器已完成清晰图放行、四类异常互斥提示及临时图片清理验收。iOS 横竖屏、闪光灯模式和拒绝权限分支仍待补测。移动 B3 尚未通过发布验收，裁切、透视、阴影、局部反光、自动旋转校正、真值标注和准确率统计仍未完成。
...
- 质量问题按过暗、过亮、模糊、倾斜的顺序返回，单张图片只提供一个当前最可操作的提示。13 张探索图的正常清晰度最低约为 788、主轴最大偏角约为 7.1 度，确定性强模糊变体最高约为 22、20 度旋转变体最小测得偏角约为 16 度；当前阈值刻意留出间隔，但仍需真实多条件拍摄真值验证。
...
- 裁切、透视、阴影、局部反光及更复杂的局部或运动模糊检测；当前只覆盖严重整体曝光异常、明显全局模糊和明显画面倾斜。
- 彩票边缘与裁切检测、自动旋转、透视校正、增强候选图和号码区域二次识别；当前倾斜能力只负责阻断，不会修改原图。
...
- 自动化测试共 515 项：JVM 180 项、Android Host 170 项、iOS Simulator 165 项；失败、错误和跳过均为 0。新增覆盖 0 度清晰图、8 度轻微倾斜放行、20 度明显倾斜拒绝，以及稀疏斜边和无序纹理的保守放行。
- Android Debug APK：62,129,674 字节，SHA-256 `8ed2dc0d3c92ecf2d6d297e2f13c115dc84381c63a24360956930ed9da14fffd`。
- Xcode `iosApp` Scheme 已针对 iPhone 17 Pro、iOS 26.5 Simulator 完整构建成功；当前 `.app` 安装后完成清晰图放行及四类图片质量异常运行验收。
