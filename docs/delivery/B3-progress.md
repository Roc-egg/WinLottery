...
- 一张此前两端均无法形成草稿的五注多期大乐透探索图已完成专项复验。两端原生 OCR 均能提供全部关键字段；共享解析修复标题、分区符和纵排文字干扰后，Android 16 与 iOS 26.5 模拟器均自动形成完整五注草稿。验收仅使用无元数据副本，真实 OCR 全文和票面字段未写入仓库。
...
- 自动化测试共 524 项：JVM 181 项、Android Host 174 项、iOS Simulator 169 项；失败、错误和跳过均为 0。新增覆盖四侧印刷内容贴边裁切、受损 OCR 摘要、漏分区符和纵排文字跨行干扰。
...
3. 补齐大乐透单期基本票、单期追加票及当前双色球版式样本，并建立逐 token 双人脱敏真值台账。
